...
public class StarRush : MonoBehaviour
{
    ...
    if (StarCount == 3)
    {
        anim.SetBool("Abrir Puerta", true);  // Establece el parñmetro de animaciñn "Abrir Puerta" en verdadero
    }

    if ((btn || Input.GetKeyDown(KeyCode.Return)) && CtoPuerta && StarCount >= 5)//Condicion final para ganar
    {
        StartCoroutine(Victory(1.5f));  // Inicia la corrutina "Victory" despuñs de cumplir ciertas condiciones
    }
    }
